...
public class ReviewsRepositoryImpl implements ReviewsRepository {
    ...
    @Override
    public List<Reviews> getReviews(int storesId, int page) {
        ...
        query = query.where(builder.equal(root.get("storesId"), storesId));
        query = query.orderBy(builder.desc(root.get("id")));
        
        Query q = s.createQuery("From Comment Where storesId=:id");
        int max=5;
        q.setMaxResults(max);
        q.setFirstResult((page-1)*max);
        q.setParameter("id", storesId);
